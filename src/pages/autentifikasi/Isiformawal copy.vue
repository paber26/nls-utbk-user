...
onMounted(async () => {
  try {
    ...
    if (sekolahRes && sekolahRes.data) {
      sekolahList.splice(0, sekolahList.length, ...sekolahRes.data)
    }
  } catch (err) {
...
const filteredSekolah = computed(() => {
  if (!sekolahQuery.value || sekolahQuery.value.trim().length < 2) return []
  ...
  return sekolahList
    .filter((s) => {
      ...
      // setiap kata yang diketik harus ada di nama sekolah
